...
from ..microserviceconstnclass import SEPARATOR, PROPAGATION_DELAY, ValidationError, MicroserviceError
...
def setupTmpBaseDir(baseDirPath=""):
    ...
    if not baseDirPath:
        baseDirPath = tempfile.mkdtemp(prefix="microservice",  suffix="test", dir="/tmp")
...
def verify64ub(signature, message, verkey):
    """
    Returns True if signature is valid for message with respect to verification
    key verkey
...
def makeUserReg(vk, sk, changed=None, username=None, **kwa):
    """
    Return duple of (signature,registration) of minimal self-signing
    agent registration record for keypair vk, sk
    ...
    if not username:
        raise MicroserviceError('Invalid username')
    
    vk64u = base64.urlsafe_b64encode(vk).decode("utf-8")

    reg = ODict(username=username, key="{}".format(vk64u), changed="")  # create registration record as dict
    if kwa:
        reg.update(kwa.items())
...
def signRequest(sk, request):
    sig = libnacl.crypto_sign(request, sk)[:libnacl.crypto_sign_BYTES]
    signature = keyToKey64u(sig)
    return signature
