...
from ..microserviceconstnclass import (SEPARATOR, PROPAGATION_DELAY, MicroserviceError, DID_LENGTH,
...
DATABASE_DIR_PATH = "/var/microservice/db"  # default
ALT_DATABASE_DIR_PATH = os.path.join('~', '.microservice/db')
...
def setupDbEnv(baseDirPath=None):
    ...
    gDbEnv.open_db(b'raw_sig')
    gDbEnv.open_db(b'unprocessed')
    gDbEnv.open_db(b'reputation')
    gDbEnv.open_db(b'rids')
    gDbEnv.open_db(b'users')
    return gDbEnv
...
def setupTestDbEnv():
    ...
    baseDirPath = os.path.join(baseDirPath, "db/microservice")
    os.makedirs(baseDirPath)
    return setupDbEnv(baseDirPath=baseDirPath)

def putUnprocessed(req = None, sig = None, env = None):
    if req == None:
        return
    ...
    #rawDB = env.open_db(dbn.encode("utf-8"))  # open named sub db named dbn within env
    unprocessedDB = env.open_db(dbn2.encode("utf-8"))
    unpIndex = None

    with env.begin(db=unprocessedDB) as txn:
        unpIndex = txn.get(b'index')
        if unpIndex:
            unpIndex = int(unpIndex)
        else: 
            unpIndex = 0

    unpIndex = unpIndex + 1

    with env.begin(write=True, db=unprocessedDB) as txn:
        txn.put(b'index', bytes(str(unpIndex), "utf-8"), dupdata=False, overwrite=True)

    rawDB = env.open_db(dbn.encode("utf-8"))  # open named sub db named dbn within env
    dbRawSig = env.open_db(b'raw_sig')
    idx = str(unpIndex).encode("utf-8")
    with env.begin(write=True, db=rawDB) as txn:
        txn.put(idx, req)

    with env.begin(write=True, db=dbRawSig) as txn:
        txn.put(idx, sig.encode("utf-8"))

def RegisterUser(username,  key):
    global gDbEnv
    ...
    dbUsers = env.open_db(b'users')

    with env.begin(db=dbUsers, write=True) as txn:
        result = txn.put(username.encode("utf-8"), key.encode("utf-8"), overwrite=False)
        return result
...
def processRecord(env = None):
    dbn = "raw"
    dbn2 = "unprocessed"
    db_rids = b'rids'
    db_reputation = b'reputation'
    ...
    dbUnprocessed = env.open_db(dbn2.encode("utf-8"))
    unpIndex = None

    with env.begin(db=dbUnprocessed) as txn:
        unpIndex = txn.get(b'index')
        if unpIndex:
            unpIndex = int(unpIndex)
        else: 
    ...
    dbRaw = env.open_db(dbn.encode("utf-8"))  # open named sub db named dbn within env
    dbReputation = env.open_db(db_reputation)
    sig = None
    req_content = None
    dbRawSig = env.open_db(b'raw_sig')
    dbUsers = env.open_db(b'users')
    ...
    if (unpIndex > 0):
        ...
        with env.begin(db=dbRaw) as txn:
            req_content = txn.get(idx)

        with env.begin(db=dbRawSig) as txn:
            sig = txn.get(idx)

    if req_content is not None:
        doc = None
        try:
            doc = json.loads(req_content)#json.loads(req.stream.read().decode('utf-8'));
            username = doc['reputee']
            rid = doc['repute']['rid']
            print("username = ", username)
            print("rid = ", rid)
            feature = doc['repute']['feature']
            value = doc['repute']['value']
            ridsDb = env.open_db(db_rids)

        except KeyError:
            print("unpIndex = ", unpIndex)
            print("record = ", req_content)
            txn = env.begin(db=dbRaw, write=True)
            removeRecord(unpIndex, txn, dbRaw, dbRawSig, dbUnprocessed)
            txn.commit()
            return

        key = None

        with env.begin(db=dbUsers) as txn:
            key = txn.get(username.encode("utf-8"))

        if key is None:
            print("no verkey found")
        ...
        try:
            if helping.verify64ub(sig.decode("utf-8"), req_content, key.decode("utf-8")):
        ...
        with env.begin(db=ridsDb, write=True) as txn:
            rids = txn.get(username.encode("utf-8"))
            print("rids = ", rids)
            try:
                if rids is not None:
                ...
                if rid in rids:
                    ...
                    rids.append(rid)
                    txn.put(username.encode("utf-8"), json.dumps(rids).encode("utf-8"))
                    reputation = txn.get(username.encode("utf-8"), db=dbReputation)
                    print("reputation1 = ", reputation)
                    a = 2
                    b = 6
                    if feature == 'clarity':
                        a = 4
                    ...
                    if reputation is not None:
                        try:
                            ...
                            reputation[feature]
                            requests = reputation[feature]['requests']
                            old_score = reputation[feature]['score']
                            old_confidence = reputation[feature]['confidence']
                            new_requests = (requests + 1)
                            score = ((old_score*requests) + value)/new_requests
                            reputation[feature]['score'] = score
                            reputation[feature]['requests'] = new_requests
                            reputation[feature]['confidence'] = S(a, b, new_requests)

                        except KeyError:
                            reputation[feature] = {}
                            reputation[feature]['score'] = value
                            reputation[feature]['requests'] = 1
                            reputation[feature]['confidence'] = S(a, b, 1)
                        ...
                        try:
                            ...
                            reach_score = reputation['reach']['score']
                            reach_confidence = reputation['reach']['confidence']
                            clarity_score = reputation['clarity']['score']
                            clarity_confidence = reputation['clarity']['confidence']
                        except KeyError:
                            pass

                        reputation['clout'] = {}
                        reputation['clout']['score'] = numpy.float64(0.1*(reach_score*reach_confidence + clarity_score*clarity_confidence))/float(reach_confidence + clarity_confidence)
                        reputation['clout']['confidence'] = min(reach_confidence, clarity_confidence)
                    ...
                    txn.put(username.encode("utf-8"), reputation.encode("utf-8"), db=dbReputation)
                    removeRecord(unpIndex, txn, dbRaw, dbRawSig, dbUnprocessed)
            except KeyError:
                txn.delete(username.encode("utf-8"))
                rids = None

            if rids is None:
                rids = [rid,]
                print(json.dumps(rids).encode("utf-8"))
                status = txn.put(username.encode("utf-8"), json.dumps(rids).encode("utf-8"))
                print('status =' + str(status))

def removeRecord(unpIndex, txn, dbRaw, dbRawSig, dbUnprocessed):
    idx = str(unpIndex).encode("utf-8")
    print('idx = ', idx)
    result = txn.delete(idx, db=dbRaw)
    result = txn.delete(idx, db=dbRawSig)
    print('result = ', result)
    print (unpIndex)
    result = txn.put(b'index', str(int(unpIndex)-1).encode("utf-8"), db=dbUnprocessed, dupdata=False, overwrite=True);
...
def getReputation(username):
    ...
    ret_val = None
    dbReputation = env.open_db(b'reputation')
    with env.begin(db=dbReputation) as txn:
        ret_val = txn.get(username.encode("utf-8"))
